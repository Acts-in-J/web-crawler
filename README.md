...
이 도구는 차단된 사이트에 대해 **우회를 시도할 수 있는 능력을 제공하지만, 그 지점에 닿을 때마다
한 번 확인을 구합니다** (그 프로필이 이미 확인 기록을 들고 있으면 면제). 진행 여부의 판단과 그
...
판단의 주체를 도구에서 사용자로 옮기는 것이 이 확인 절차의 목적입니다. 다만 이는 **도구의 동작**이고, 실제로 실행하는 AI 에이전트는 자체 판단 기준을 따로 갖습니다 — [`ACCEPTABLE_USE.md`](ACCEPTABLE_USE.md) 참조.
...
> **이미 `fingerprints/` 를 갖고 있다가 이 버전으로 올린 경우.** 확인 절차를 넘어선 방법(예: `chrome_cdp`, `stealthy`)이 적힌 기존 프로필에 `consent` 기록이 없으면, 다음 저장에서 `ConsentRequired` 로 한 번 멈춥니다. **버그가 아니라 설계된 동작입니다** — 예전에는 그런 프로필이 조용히 배포 대상으로 분류됐고, 이제는 사용자에게 한 번 알린 사실이 있어야 저장이 끝납니다. 알리고 '진행' 을 고른 뒤 그 시각과 선택을 `consent` 에 적으면 이어서 진행되고, **그 기록을 들고 있는 동안은** 다시 묻지 않습니다.

> **다시 묻는 경우도 있습니다.** 확인이 면제되는 근거는 도메인이 아니라 프로필이 지금 들고 있는 `consent` 기록입니다. 어떤 사이트가 한동안 차단을 풀어 평범한 방법으로 수집되면 그 프로필은 배포 대상이 되고, 그때 `consent` 는 지워집니다 — 사용자가 언제 무엇을 확인받았는지를 배포되는 파일에 실어 내보내지 않기 위해서입니다. 그 사이트가 나중에 새 보호를 걸면 확인을 한 번 더 구합니다. **사이트가 달라진 것은 달라진 상황이고, 같은 지점을 다시 지나는 것은 새로운 일입니다.**
...
> 기준을 따로 가지며 이 문서가 그것을 대신 약속하지 않습니다 — [`ACCEPTABLE_USE.md`](ACCEPTABLE_USE.md) 참조.
