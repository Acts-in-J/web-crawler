...
| | 무엇인가 | 어긴다는 것은 |
|---|---|---|
| **약관·robots** | 사이트가 "이렇게 써 주세요" 라고 내건 **이용 조건**. 그 사이트를 쓰는 순간 동의한 것으로 취급됩니다 | **약속을 깬 것** — 계정 정지, 중단 요구, 손해배상 청구로 이어집니다 |
...
- **가장 자주 빠뜨리는 것은 ④ 분량입니다.** 한 번에 많이 가져가지 않아도, **매일 조금씩 꾸준히** 가져가면 많이 가져간 것으로 봅니다 — 하루 10건씩 1년이면 그건 10건이 아닙니다. 기준은 두 가지이고 **둘 중 하나만** 걸려도 해당합니다: (a) 원래 사이트를 이용하는 것과 충돌하거나, (b) 그 DB 를 만든 쪽의 이익을 부당하게 해치거나. (저작권법 §93②)
- **⑥ 흔적은 법이 요구하는 것은 아니지만 실무에서 효과가 가장 큽니다.** User-Agent 에 신원과 연락처를 남기는 것만으로 "숨길 생각이 없었다" 는 증거가 되고, 실제로 대부분의 사이트는 자기를 밝히는 봇을 그냥 둡니다.

### 3. 자주 하는 오해

**Q. 공개된 페이지니까 마음껏 긁어도 되나요?**
A. "공개" 는 **기술적 장벽이 없다**는 뜻이지 **약관이 허용한다**는 뜻이 아닙니다.
미국 hiQ 사건에서 법원은 공개 프로필 수집에 해킹법(CFAA)까지 적용되지는 않는다고 보면서도
**이용약관 위반은 별개로 인정**했고, hiQ 는 50만 달러를 내고 마무리했습니다.
기술적으로 열려 있다는 것과 계약상 허용된다는 것은 서로 다른 층입니다.

**Q. 개인정보만 아니면 되나요?**
A. 아닙니다. 누구나 아는 사실 정보라도 **체계적으로 모아둔 집합에는 별도의 권리**가 붙습니다.
...
형사 층은 접근·부담뿐 아니라 개인정보에도 걸쳐 있어서 "내부용이니 형사는 상관없다" 는
...
- **저작권법의 "사적이용"(§30) 은 회사 내부를 포함하지 않습니다.** 조문은 *"영리를 목적으로
  ...
  **둘 다** 요구합니다. 뒤쪽은 "개인적 이용" 이나 "가정에 준하는 한정된 범위" 중 **하나만**
  채우면 되는 선택지지만, 회사 내부의 조직적 복제는 앞 요건과 뒤 선택지 **양쪽 모두에서
  걸립니다** — 통설과 실무는 **기업 내부의 조직적 복제를 여기서 배제**합니다. "회사 내부" 는
  "사적" 의 확장이 아니라 정반대입니다.
- **부정경쟁방지법은 "영리" 가 아니라 "자신의 영업을 위하여" 를 봅니다.** 경쟁사 가격을 긁어
  내부 가격 결정에 썼다면, 팔지 않았어도 명백히 영업을 위한 것입니다.
- **개인정보보호법은 영리 여부를 아예 묻지 않습니다.**

내부용이라서 실제로 낮아지는 것은 **배상액 산정**과 **발각 가능성**이지 위법성 자체가 아닙니다.

### 4. 이 도구가 하는 일

robots.txt 는 법적 구속력이 없지만 표지판입니다. 무시했다는 사실은 "알고도 했다" 의 정황이 됩니다.

이 도구는 차단된 사이트에 대해 **우회를 시도할 능력을 갖고 있지만, 그 지점에 닿을 때마다
사용자에게 한 번 확인을 구합니다** (그 프로필이 이미 확인 기록을 들고 있으면 생략). 진행할지
말지의 판단과 그 결과에 대한 책임은 사용자에게 있습니다.

**막히면 여기로 돌아오세요.** 여섯 중 무엇이 빠졌는지 짚고, 채울 수 있으면 채우고,
못 채우면 논란 없는 영역의 대안(공식 API·공개 데이터 포털)을 먼저 찾습니다.
...
> **`py` 런처가 깨져 있어도 알아서 처리합니다.** 일부 Windows 에서는 `py -3` 가 `No installed Python found!` 로 실패합니다. `setup.ps1` 은 `py -3`/`python`/`python3` 를 **실제로 실행해 3.10+ 인지 확인**하고, 성공하는 쪽으로 `.venv` 를 만듭니다 — `py -3` 가 실패하면 자동으로 `python` 으로 넘어갑니다.
> 설치 도중 pip 이 한동안 조용해서 멈춘 것처럼 보여도 정상입니다(용량 큰 휠을 받는 중). 진행 로그를 보려면 `-VerbosePip` 를 붙이세요(예: `... -File scripts\setup.ps1 -VerbosePip`).
...
Windows PowerShell 기준 — 그대로 붙여 넣으면 동작하는 명령입니다.
...
**꼭 알아둘 점**
- `python -m scrapling` 은 **동작하지 않습니다**(`__main__` 이 없습니다). venv 를 활성화한 뒤 `scrapling install`, 또는 `.\.venv\Scripts\scrapling.exe install` 을 쓰세요.
- `scrapling install` 이 내부에서 `playwright install chromium` 까지 해 줍니다. **`playwright install` 을 따로 또 돌리지 마세요** — 같은 파일을 다시 내려받느라 시간만 버립니다. 이미 받아둔 상태면 즉시 끝납니다.
- PowerShell 에서 `npm` / `agent-browser` 가 실행 정책 오류를 내면 **`npm.cmd` / `agent-browser.cmd`** 로 바꿔 실행하세요.
- 검증만 다시 하려면 `python scripts\preflight.py` (core만 보려면 `--core-only`).
...
> macOS / Linux 는 위 명령에서 `py -3 -m venv` → `python3 -m venv`, 활성화 `. .venv/bin/activate`, `npm.cmd` → `npm`, `agent-browser.cmd` → `agent-browser` 로 바꾸면 동일합니다.
...
> **왜 정찰에만 Node 런타임(agent-browser)이 따로 필요한가.** Playwright 는 Scrapling 을 깔 때 이미 함께
> 들어옵니다. 그런데도 agent-browser 를 쓰는 이유는, 이 도구가 AI 에이전트가 다루기 좋은 형태(snapshot/ref)로
> 설계돼 있어 **같은 정찰을 훨씬 적은 토큰으로** 끝내기 때문입니다. 두 번째 런타임을 설치하는 비용을 주고
> 그 편의를 산 것이고, **의식적인 선택**입니다. 수집에는 쓰지 않습니다 — 정찰과 수집은 분리돼 있습니다.
...
> **소프트블록**은 사실상 차단인데 겉보기에는 성공(HTTP 200)처럼 오는 응답입니다. 빈 껍데기 페이지를 정상 데이터로 착각하고 계속 긁으면 차단이 굳어지므로, 다른 검증보다 **먼저** 확인합니다.
...
수집이 실패하면 **가벼운 방법부터** 차례로 자동 전환합니다 — `plain_get → plain_session → DynamicFetcher`. 여기까지는 사이트가 나를 막은 것이 아니라 **데이터가 놓여 있는 위치가 다를 뿐**이라, 묻지 않고 자동으로 진행합니다.

**그 위부터는 자동으로 넘어가지 않습니다.** 사이트가 자동 접근을 차단하고 있다면(봇 탐지·WAF·CAPTCHA), 에이전트가 **한 번 알리고 진행 여부를 묻습니다.** '진행' 을 고르면 그대로 진행하며 — 근거를 제출받거나 검증하지 않습니다 — 그 선택은 도메인 프로필에 기록됩니다. 그 프로필이 그 기록을 들고 있는 동안은 다시 묻지 않습니다.

이 확인 절차의 목적은 판단의 주체를 도구에서 사용자로 옮기는 것입니다. 다만 이는 **도구의 동작**이고, 실제로 실행하는 AI 에이전트는 자체 판단 기준을 따로 갖습니다 — [`ACCEPTABLE_USE.md`](ACCEPTABLE_USE.md) 참조.

---

## 도메인 프로필 (재수집 가속)

같은 사이트를 다시 수집할 때 정찰을 건너뛸 수 있도록, 수집에 성공하면 `fingerprints/<도메인>/profile.json` 에 "수집 레시피" 를 저장합니다. 다음번에는 이 레시피만 보고 바로 수집에 들어갑니다.
...
- **저장은 필수.** Step 5-A 게이트 — 빠뜨리면 다른 머신이나 다음 세션에서 그 노하우가 통째로 사라진다.
- **`notes` 를 비우지 않기.** "이 사이트는 브라우저 세션이 필요 — chrome_cdp", "review API는 HTML 반환" 처럼, 다음 사람이 정찰 없이 바로 출발하게 해 주는 결정적 메타 정보다.
- **자격증명은 넣지 않기.** 배포되는 profile.json 은 commit 대상이므로 API key·토큰·쿠키는 반드시 별도 파일로 분리한다.
- **확인을 거친 프로필은 그 선택도 함께 기록한다.** 자동 접근 차단을 넘어선 방법으로 수집했다면 `consent` 블록(알린 시각과 사용자의 선택)이 함께 저장된다. 무엇을 정당화했는지가 아니라 **알렸고 사용자가 골랐다는 사실**만 남는다.

> **예전 `fingerprints/` 를 갖고 있다가 이 버전으로 올린 경우.** 확인 절차를 넘어선 방법(예: `chrome_cdp`, `stealthy`)이 적힌 기존 프로필에 `consent` 기록이 없으면, 다음 저장에서 `ConsentRequired` 로 한 번 멈춥니다. **버그가 아니라 설계된 동작입니다** — 예전에는 그런 프로필이 조용히 배포 대상으로 분류됐지만, 이제는 사용자에게 한 번 알린 사실이 있어야 저장이 끝납니다. 알리고 '진행' 을 고른 뒤 그 시각과 선택을 `consent` 에 적으면 이어서 진행되고, **그 기록을 들고 있는 동안은** 다시 묻지 않습니다.

> **다시 묻는 경우도 있습니다.** 확인이 면제되는 근거는 도메인이 아니라 프로필이 지금 들고 있는 `consent` 기록입니다. 어떤 사이트가 한동안 차단을 풀어서 평범한 방법으로 수집되면 그 프로필은 배포 대상이 되고, 그때 `consent` 는 지워집니다 — 사용자가 언제 무엇을 확인받았는지를 배포되는 파일에 실어 내보내지 않기 위해서입니다. 그 사이트가 나중에 새 보호를 걸면 확인을 한 번 더 구합니다. **사이트가 달라진 것은 달라진 상황이고, 같은 지점을 다시 지나는 것은 새로운 일입니다.**
...
**이 레포 하나가 전부입니다.** 따로 숨겨둔 내부 저장소는 없습니다. 커밋되는 것은 코드와 수집 레시피뿐이고, **실제로 수집한 데이터는 어떤 경로로도 커밋되지 않습니다.**
...
- **자동 접근 차단을 만나면 한 번 확인** — CAPTCHA·WAF·봇 탐지를 만나면 자동으로 넘어가지 않고 사용자에게 알린 뒤 묻습니다. *능력이 없어서가 아니라, 판단의 주체가 사용자이기 때문입니다.* '진행' 을 고르면 그대로 진행합니다
...
- **수집한 데이터를 어디에 쓸지는 사용자 책임** — 이 도구는 수집을 도울 뿐, 수집한 뒤의 재배포·경쟁 서비스·학습데이터 이용에 대해서는 판단하지 않습니다. *"어떻게 들어갔나" 와 "가져다 뭘 했나" 는 다른 질문이고, 답하는 법도 서로 다릅니다*

> **'경고' 와 '확인' 은 같은 층위입니다.** 경고는 요청이 **어떤 위험을 안고 있는지** 알리는 것이고,
> 확인은 수집 도중 만난 **기술적 차단**을 알리는 것입니다. 둘 다 알리는 데서 끝나고 고르는 쪽은
> 사용자입니다 — 어느 쪽도 요청 자체를 막지 않고, 근거를 제출받거나 검증하지 않습니다.
>
> 다만 이 문서가 정의하는 것은 **도구의 동작**입니다. 실제로 실행하는 AI 에이전트는 자체 판단
> 기준을 따로 가지며 이 문서가 그것을 대신 약속하지는 않습니다 — [`ACCEPTABLE_USE.md`](ACCEPTABLE_USE.md) 참조.
...
2. **fingerprints 배포 화이트리스트 (default-deny)** — `fingerprints/**` 를 통째로 ignore 한 다음, **배포 대상으로 판정된 프로필만** 명시적으로 whitelist 합니다. 이 목록은 `scripts/profile_policy.py` 의 판정 결과를 받아 `scripts/sync_domain_list.py` 가 생성합니다. 자동 접근 차단을 넘어선 방법을 기록한 프로필은 **로컬에는 남되 배포되지는 않습니다** — 능력은 그대로 있고 레시피만 빠집니다.
